...
- **vault** — for agent-written prose being saved into either of Jamie's Obsidian vaults: the personal vault at `~/workspace/vault` and the research vault at `~/workspace/research`. Substance rules are colleague mode's: keep every fact, number, file path, link, and code block intact, and don't shorten beyond what removing fluff removes. What differs is that both vaults follow `~/workspace/vault/STYLEGUIDE.md`, and a generic rewrite violates it — agy introduces em dashes and Americanises spelling unless told not to, both observed in practice. So this mode adds the STYLEGUIDE's hard constraints to the prompt (see below), and the result must satisfy both sets of rules. Not for anything in Jamie's own voice or anyone else's; see the exclusions in `vault/AGENTS.md` and `research/AGENTS.md`.
