...
license: Based on Anthropic's frontend-design skill.
---
...
2. **Check DESIGN.md (fast)**: If not in instructions, read `DESIGN.md` from the project root. If it exists and contains the required context, proceed.
3. **Run design-setup (REQUIRED)**: If neither source has context, you MUST run /design-setup NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
